...
import { Loader2, User, Lock, Shield, LogOut, ShieldCheck, ShieldOff, Camera, Trash2, Bell, Car, BarChart3, Newspaper, AlertTriangle, Unlock } from "lucide-react";
  ...
  const updateNotificationPreference = async (key: keyof NotificationPreferences, value: boolean) => {
    ...
    try {
      const { error } = await supabase
        .from("notification_preferences")
        .update({ [key]: value } as Record<string, boolean>)
        .eq("user_id", user.id);
