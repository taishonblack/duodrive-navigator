...
const sessionTypeTiers: Record<string, string> = {
  text: "text",
  phone: "phone",
  video: "concierge",
};
...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Notifying coaches for request:", requestId);
    ...
    // Determine which tier of coaches to notify
    const coachTier = sessionTypeTiers[request.session_type] || "text";
    
    // Fetch all available coaches for this tier
    ...
    if (!coaches || coaches.length === 0) {
      console.log("No available coaches for tier:", coachTier);
      return new Response(
        JSON.stringify({ success: true, message: "No available coaches to notify", notifiedCount: 0 }),
        { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders } }
      );
    }

    console.log(`Found ${coaches.length} coaches for tier ${coachTier}`);

    // Fetch email addresses for all coaches
    ...
    if (profiles) {
      profiles.forEach(p => {
        if (p.email) coachEmails[p.id] = p.email;
      });
    }
    ...
    const emailPromises = coaches.map(async (coach) => {
      ...
      if (!email) {
        console.log(`No email found for coach ${coach.display_name}`);
        return null;
      }
        ...
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 16px 16px;">
            ...
            <div style="background: #f9fafb; padding: 20px; border-radius: 12px; margin: 20px 0;">
              ...
              <p style="margin: 5px 0; color: #111827;"><strong>📅 Requested Date:</strong> ${formattedDate}</p>
              <p style="margin: 5px 0; color: #111827;"><strong>🕐 Requested Time:</strong> ${formattedTime}</p>
            ...
            <div style="text-align: center; margin: 25px 0;">
              <a href="${dashboardUrl}" style="display: inline-block; background: #8b5cf6; color: white; padding: 14px 28px; border-radius: 8px; text-decoration: none; font-weight: 600;">
                View & Claim Request
              </a>
            </div>

            <p style="font-size: 14px; color: #6b7280; text-align: center;">
              First coach to claim gets the session! Be quick! ⚡
            </p>
      ...
      try {
        const result = await resend.emails.send({
          ...
          to: [email],
          subject: `🔔 New ${sessionLabel} Request Available!`,
          html: htmlContent,
        });
        console.log(`Email sent to coach ${coach.display_name}:`, result);
        return { coach: coach.display_name, success: true };
      } catch (emailError) {
        console.error(`Failed to send email to ${coach.display_name}:`, emailError);
        return { coach: coach.display_name, success: false, error: emailError };
      }
    });
    ...
    console.log(`Notified ${successCount} coaches`);
    ...
    const errorMessage = error instanceof Error ? error.message : "Failed to notify coaches";
    return new Response(
